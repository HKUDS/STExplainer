...
class STGIB(nn.Module):
    ...
    def forward(self, history_data: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        spa_out, temp_out = self.batch_st_gib(X)
        spa_out = spa_out.transpose(2, 1)
        spa_out = spa_out.reshape(B, N, -1).transpose(1, 2).unsqueeze(-1)      # B, L*d_model, N, 1
        time_series_emb_spat = self.time_series_emb_layer(spa_out)         # B, D, N, 1
        time_series_emb.append(time_series_emb_spat)
    ...
    def batch_st_gib(self, inputs):
        ...
        if args.only_spat is not True:
            temp_in = inputs.reshape(batch_size, args.lag, -1)
